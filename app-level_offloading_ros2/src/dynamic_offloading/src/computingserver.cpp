...
public:
  ...
  : Node("computing_node_server")
  {
    ...
    publisher_2 = this->create_publisher<header_interfaces::msg::NewHeader>("Result_Server", 10);
...
private:
  ...
  void topic_callback2(const header_interfaces::msg::NewHeader::SharedPtr msg)
  {
   ...
   ////auto parameters_client = std::make_shared<rclcpp::SyncParametersClient>("computing_node_server","arbitrator");
    /////int is_local_delayed = parameters_client->get_parameter<int>("local_delayed");
    //rclcpp::Parameter pointer;
    //this->get_parameter("local_delayed", pointer);
    //auto pointer = this -> get_parameter("local_delayed");
    //auto is_local_delayed = pointer.get_value();
    //RCLCPP_INFO(this->get_logger(), "---------------------------local_delayed : %d---------------------------", is_local_delayed);
